...
We support signed-in bots for Google Meet. You'll need to create a new Google Workspace account for the bot to use. You'll then configure the workspace so that the Attendee server is an OIDC SSO Identity Provider that the bot can use to sign in. This is more reliable than signing in with a username and password.
...
3. Enter the workspace domain and email. The email must match the email of the bot user in your Google Workspace account.
4. The system will auto-generate a Client ID and Client Secret and display them once. Copy these values immediately.

#### Configure OIDC SSO in Google Workspace
1. Navigate to the Admin Console, then to Security -> Set up single sign-on (SSO) with a third party IdP.
2. Click "Add SSO profile" and select "OIDC profile".
3. In the OIDC SSO profile page, enter the following information:
    - Client ID — the Client ID generated by Attendee
    - Client secret — the Client Secret generated by Attendee
    - Issuer URL — `https://app.attendee.dev/bot_sso` (For self-hosted instances, use the hostname of your instance instead of app.attendee.dev)
    - Change password URL — (leave blank or use a placeholder)
4. Click the "Save" button.
5. Under the Manage SSO profile assignments section, click the "Manage" button.
6. Select the OIDC SSO profile you created and click the "Save" button.
